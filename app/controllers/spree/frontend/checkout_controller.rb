module Spree
  ...
  class CheckoutController < Spree::StoreController
    ...
    def edit
      default_address = current_user.default_address.try(:address)
      @addresses = current_user.addresses.where.not(id: default_address.try(:id))
      @addresses.unshift(default_address) if default_address.present?
      @order.email = default_address.try(:email)

      if ! @order.ship_address.try(:valid?) || ! @order.bill_address.try(:valid?)
        @order.ship_address_id = default_address.try(:id)
        @order.bill_address_id = default_address.try(:id)
      end

      @order.ship_address_id ||= default_address.try(:id)
      @order.bill_address_id ||= default_address.try(:id)
      if @order.bill_address_id.present? && @order.ship_address_id.present?
        @order.save!
      end
      request_all_addresses
    end
